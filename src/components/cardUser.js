  ...
  return (
    ...
    >
      ...
      {<div className="d-flex flex-wrap justify-content-center row m-0">
        ...
        >
            <InnerCard w={wi} title={"Nome e Cognome"} description={(utente?.first_name ? utente?.first_name : "-")+" "+(utente?.last_name ? utente?.last_name : "")} i={indice}></InnerCard>
        </div>
        <div
          className={"card col-sm-4 col-md-3 col-lg-3 col-xl-2 p-0 innercardorders"}
        >
            <InnerCard w={wi} title={"Email"} description={utente?.username} i={indice}></InnerCard>
        </div>
        <div
          className={"card col-sm-4 col-md-3 col-lg-2 p-0 innercardorders"}
        >
            <InnerCard w={wi} title={"Indirizzo"} description={(utente?.address ? utente.address : "-")+" "+(utente?.city ? utente.city : "")} i={indice}></InnerCard>
        </div>
        <div
          className={"card col-sm-4 col-md-3 col-lg-3 col-xl-2 p-0 innercardorders"}
        >
            <InnerCard w={wi} title={"Telefono"} description={utente?.phone_number} i={indice}></InnerCard>
        </div>
     
        <div
          className={"card col-sm-4 col-md-3 col-lg-2 p-0 innercardorders"}
        >
            <InnerCard w={wi} title={"Attivo"} description={utente?.is_active ? "Attivo" :"Disattivo"} i={indice}></InnerCard>
        </div>
        {indice!==-1 && <div
        ...
        >
          <div className="card-body p-1 row">
            ...
            <button
                ...
                onClick={()=>{navigate("/users/detail/"+utente.id_user_customer)}}
              >
